...
```{r}
#use ANOSIM to determine if ALAM and SSID were different in just hypoxic plots
# plot number of observed ASVs in coral samples
ps_hyp<-subset_samples(ps_pc, Treatment !="Control plots")

plot_richness(ps_hyp,x="Treatment",color="Species",measures=c("Observed"))
# look at data and chose filtering method for very low abundance ASVs
ntaxa(ps_hyp) #22823
ps_hyp_5<-filter_taxa(ps_hyp, function(x) mean(x) >5, TRUE)
ntaxa(ps_hyp_5) #909
get_taxa_unique(ps_hyp, "Genus") # 1000
get_taxa_unique(ps_hyp_5, "Genus") #304

# filtered ASVs with very low abundance with phyloseq, now export otu and taxa tables from phyloseq for codaseq
otu = as(otu_table(ps_hyp_5), "matrix")
taxon = as(tax_table(ps_hyp_5), "matrix")
metadata = as(sample_data(ps_hyp_5), "matrix")
write.table(otu,"Panama_pshyp5_silva_nochloronomito_otu_table.txt",sep="\t",col.names=NA)
write.table(taxon,"Panama_pshyp5_silva_nochloronomito_taxa_table.txt",sep="\t",col.names=NA)
write.table(metadata,"Panama_pshyp5_silva_metadata.txt",sep="\t",col.names=NA)

######### Perform center-log-ratio transformation on ASVs and calculate Aitchison Distance and principal components
# READ IN OTU data that has been filtered for very low abundance sequences; do not clear data here. Keep phyloseq object psfto5 for anosim/permanova
otu <- read.table("Panama_pshyp5_silva_nochloronomito_otu_table.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("Panama_pshyp5_silva_nochloronomito_taxa_table.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("Panama_pshyp5_silva_metadata.txt",sep="\t",header=T,row.names=1)
genus<-as.character(taxon$Genus)
# First, replace 0 values with an estimate (because normalization is taking log, can't have 0)
# Also transposing here, need samples as rows
d.czm <- cmultRepl(t(otu), method="CZM", label=0)
...
species_hyp<-as.character(samples$Species)
# set Aitchison distance as the distance measure for the anosim
dist.clr <- dist(E.clr)
ano.specieshyp <- anosim(dist.clr, species_hyp, permutations=999)
#pdf("Panama_SITE_EFFECT_ANOSIM.pdf",width=8.5)
plot(ano.specieshyp)
#dev.off()

##DeSeq2 to see what is different between Species SSID v. ALAM
