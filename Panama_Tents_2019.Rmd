...
regression<-read.table("PC1_values.txt",sep="\t",header=TRUE, row.names=1)

#regression
x<-c(regression$Last.Recorded.Dissolved.Oxygen.Measurement..mg.L.)
y<-c(regression$PC1)
model<-lm(x~y)
summary(model)

##graph
#pdf("LinearRegression.pdf",width=8.5)
theme_set(theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()))
p<-ggplot(regression, aes(x=x, y=y))+
  geom_point(size=3)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
  ylab("PC1") + xlab("Last Recorded Dissolved Oxygen Measurement (mg/L)")
