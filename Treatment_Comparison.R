##all of the corals at Punta were treated, so this is Treatments
...
write.table(otu,"Punta_OTU.txt",sep="\t",col.names=NA)
write.table(taxon,"Punta_taxa.txt",sep="\t",col.names=NA)
write.table(metadata,"Punta_metadata.txt",sep="\t",col.names=NA)
otu <- read.table("Punta_OTU.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("Punta_taxa.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("Punta_metadata.txt",sep="\t",header=T,row.names=1)
OTU = otu_table(otu, taxa_are_rows=FALSE)
taxon<-as.matrix(taxon)
TAX = tax_table(taxon)
sampledata = sample_data(samples)
...
pdf("PCA_Punta_species_treatment.pdf",width=8.5)
