##species comparison of ALAM and SSID with unmanipulated, natural corals
...
##subset sites based on treatments, ps is phyloseq object-we just replaced it 
ps=subset_samples(ps,Site =="Finca")
ps
otu = as(otu_table(ps), "matrix")
taxon = as(tax_table(ps), "matrix")
metadata = as(sample_data(ps), "matrix")
write.table(otu,"Finca_spcies_OTU.txt",sep="\t",col.names=NA)
write.table(taxon,"Finca_spcies_taxa.txt",sep="\t",col.names=NA)
write.table(metadata,"Finca_spcies_metadata.txt",sep="\t",col.names=NA)
otu <- read.table("Finca_spcies_OTU.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("Finca_spcies_taxa.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("Finca_spcies_metadata.txt",sep="\t",header=T,row.names=1)
OTU = otu_table(otu, taxa_are_rows=FALSE)
taxon<-as.matrix(taxon)
TAX = tax_table(taxon)
sampledata = sample_data(samples)
...
pdf("PCA_Finca_species.pdf",width=8.5)
