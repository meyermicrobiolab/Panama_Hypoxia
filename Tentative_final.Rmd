...
datm <- psmelt(dat)
#Cast the new datatable with columns that are of interest
datc <- data.table::dcast(datm, Sample + Logger.MiniDOT + Logger.MiniDOT + Origin + Species ~ Family, value.var = 'Abundance', fun.aggregate = sum)
dim(datc) #dimensions of the table
#683 families
otud <- datc[,c(1,7:683)] #select the first column, and then all of the taxa columns  
colnames(otud)[1] <- "Sample.ID" #rename the first column to Sample.ID - this is to match ANCOM syntax
...
cols<-c("1"="#C9A456","2"="#E27741","3"="#ABC9CB","4"="#CB4E07","5"="#8A9B5E","6"="#643830","7"="#DAB576","8"="#B04029")
sum_sig$Logger.MiniDOT<-factor(sum_sig$Logger.MiniDOT, levels=c("1","2","3","4","5","6","7","8"))
...
sub_pc<-data.frame(Treatment = pc$Treatment, ASV_17 = pc$ASV_17, ASV_256 = pc$ASV_256, ASV_314 = pc$ASV_314, ASV_425 = pc$ASV_425)
