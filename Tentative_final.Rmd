...
#Treatment
ano.treat <- anosim(dist.clr, treat, permutations=999)
pdf("Panama_ANOSIM_treat.pdf",width=8.5)
plot(ano.treat)
dev.off()
#permanova, treatment and species
perm<-adonis2(dist.clr~treat*species,as(sample_data(ps),"data.frame"))
print(perm)
```
...
sample_data(ps_ra_alam)$Treatment<-factor(sample_data(ps_ra_alam)$Treatment,levels=c("Untreated","Control plots","Hypoxia plots"))
...
p1
##put treatments in a different order
sample_data(ps_ra_ssid)$Treatment<-factor(sample_data(ps_ra_ssid)$Treatment,levels=c("Untreated","Control plots","Hypoxia plots"))
...
#602 families
otud <- datc[,c(1,7:602)] #select the first column, and then all of the taxa columns  
...
print(sig_sites)
#Family_XII, Marinifilaceae, Parvibaculaceae, Pseudomonadaceae, Marinobacteraceae, Nitrincolaceae, Francisellaceae, Shewanellaceae, Halomonadaceae
#Calculate relative abundance
datc_relabund <-  sweep(datc[,7:602], 1, rowSums(datc[,7:602]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
sig_dis <- select(datc_relnames, Sample, Species, Treatment, Family_XII, Marinifilaceae, Parvibaculaceae, Pseudomonadaceae, Marinobacteraceae, Nitrincolaceae, Francisellaceae, Shewanellaceae, Halomonadaceae)
...
print(sig_sites)
#Family_XII, Marinifilaceae, Parvibaculaceae, Pseudomonadaceae, Marinobacteraceae
datc_relabund <-  sweep(datc[,7:602], 1, rowSums(datc[,7:602]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
sig_dis <- select(datc_relnames, Sample, Species, Treatment, Family_XII, Marinifilaceae, Parvibaculaceae, Pseudomonadaceae, Marinobacteraceae)
sig_long <- melt(sig_dis, id.vars=c("Sample","Species","Treatment"),variable.name="Family",value.name="Proportion")
library(Rmisc)
sum_sig <- Rmisc::summarySE(sig_long, measurevar = "Proportion", groupvars = c("Treatment","Family"), na.rm=TRUE)
cols<-c("Untreated"="#E69F00","Control plots"="#56B4E9","Hypoxia plots"="#CC79A7")
...
cols<-c("Untreated"="#56B4E9","Control plots"="#E69F00","Hypoxia plots"="#CC79A7")
sum_sig$Treatment<-factor(sum_sig$Treatment, levels=c("Untreated","Control plots","Hypoxia plots"))
fams <- ggplot(sum_sig, aes(x=Family, y=Proportion))+
  geom_point(size=4,aes(color=Treatment))+
  scale_colour_manual(values=cols)+
...
dim(datc) #dimensions of the table
#601 families
otud <- datc[,c(1,7:601)] #select the first column, and then all of the taxa columns  
colnames(otud)[1] <- "Sample.ID" #rename the first column to Sample.ID - this is to match ANCOM syntax
metadat <- sample_data(ps_ssid) #get the sample data
metadat <- as.data.frame(as.matrix(metadat)) #make into into a matrix
# at this point, my sample id numbers are the row names, not a separate column, move row names to column with dplylr
...
sig_sites <- glue::glue_collapse(droplevels(factor(res2$otu.names)), sep = ", ") #this is to get a list of the families that are different
print(sig_sites)
#Family_XII, Marinifilaceae, Rhodospirillales, Shewanellaceae, Marinobacteraceae, Pseudomonadaceae, Clostridiaceae_1, Nitrincolaceae, Halomonadaceae, Holosporaceae, Desulfovibrionaceae, Parvibaculaceae, Francisellaceae, Midichloriaceae, Proteobacteria, Clostridiales, Syntrophobacteraceae, Peptostreptococcaceae, Thermoanaerobaculaceae, Thermodesulfovibrionia, Bacteroidales, Cellvibrionaceae, Marinilabiliaceae, Sneathiellaceae, Nostocaceae, Desulfobacteraceae, Gammaproteobacteria, Cellvibrionales, Archaea, Idiomarinaceae, Clostridiaceae_4
#Calculate relative abundance
datc_relabund <-  sweep(datc[,7:601], 1, rowSums(datc[,7:601]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
sig_dis <- select(datc_relnames, Sample, Species, Logger.MiniDOT, Family_XII, Marinifilaceae, Rhodospirillales, Shewanellaceae, Marinobacteraceae, Pseudomonadaceae, Clostridiaceae_1, Nitrincolaceae, Halomonadaceae, Holosporaceae, Desulfovibrionaceae, Parvibaculaceae, Francisellaceae, Midichloriaceae, Proteobacteria, Clostridiales, Syntrophobacteraceae, Peptostreptococcaceae, Thermoanaerobaculaceae, Thermodesulfovibrionia, Bacteroidales, Cellvibrionaceae, Marinilabiliaceae, Sneathiellaceae, Nostocaceae, Desulfobacteraceae, Gammaproteobacteria, Cellvibrionales, Archaea, Idiomarinaceae, Clostridiaceae_4)
...
cols<-c("1"="#C9A456","2"="#E27741","3"="#1A5276","4"="#CB4E07","5"="#8A9B5E","6"="#643830","7"="#DAB576","8"="#B04029", "9"="#ABC9CB")
sum_sig$Logger.MiniDOT<-factor(sum_sig$Logger.MiniDOT, levels=c("1","2","3","4","5","6","7","8","9"))
...
sig_sites <- glue::glue_collapse(droplevels(factor(resALL4$otu.names)), sep = ", ") #this is to get a list of the families that are different
print(sig_sites)
#Family_XII, Marinifilaceae, Rhodospirillales, Shewanellaceae, Marinobacteraceae, Pseudomonadaceae, Clostridiaceae_1, Nitrincolaceae, Halomonadaceae, Holosporaceae, Desulfovibrionaceae, Parvibaculaceae, Francisellaceae, Midichloriaceae, Proteobacteria, Clostridiales, Syntrophobacteraceae, Peptostreptococcaceae, Thermoanaerobaculaceae, Thermodesulfovibrionia, Bacteroidales, Cellvibrionaceae, Marinilabiliaceae, Sneathiellaceae, Nostocaceae, Desulfobacteraceae, Gammaproteobacteria, Cellvibrionales, Archaea, Idiomarinaceae, Clostridiaceae_4, Thiotrichaceae, Litoricolaceae, Spirochaetaceae, Arcobacteraceae, Cyanobacteria, Marine_Group_II, Flammeovirgaceae, Firmicutes, B2M28, Prolixibacteraceae, OCS116_clade, Schekmanbacteria, Desulfarculaceae, Terasakiellaceae, Micrococcaceae
#Calculate relative abundance
datc_relabund <-  sweep(datc[,7:602], 1, rowSums(datc[,7:602]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
...
sig_dis <- select(datc_relnames, Sample, Species, Logger.MiniDOT,Family_XII, Desulfovibrionaceae, Arcobacteraceae, Midichloriaceae)
sig_long <- melt(sig_dis, id.vars=c("Sample","Species","Logger.MiniDOT"),variable.name="Family",value.name="Proportion")
sum_sig <- Rmisc::summarySE(sig_long, measurevar = "Proportion", groupvars = c("Species","Logger.MiniDOT","Family"), na.rm=TRUE)
#position=position_jitter(height=.00001)
...
abund = asv[,5:ncol(asv)]
treatment=asv$Treatment
...
##Next, we subset the data as desired by calling the specific columns we want to include in our boxplot figure. I’ve chosen the top four most significant ASVs associated with the “Hypoxia plots” grouping (stat-value>.55)
sub_pc<-data.frame(Treatment = pc$Treatment, Alteromonas = pc$ASV_17, Neptuniibacter = pc$ASV_256, Aestuariicella = pc$ASV_314, Marinobacter = pc$ASV_425)

##Next, we convert the data frame from a “wide” format to a “long” format 
sub_pc_m = melt(sub_pc, id = "Treatment")

##Now we can plot the data using geom_boxplot from ggplot2
sub_pc_m$Treatment<-factor(sub_pc_m$Treatment, levels=c("Untreated","Control plots","Hypoxia plots"))
cols<-c("Untreated"="#56B4E9","Control plots"="#E69F00","Hypoxia plots"="#CC79A7")
pdf("INDIC_ASVs_Treatment_boxplot.pdf",width=10)
