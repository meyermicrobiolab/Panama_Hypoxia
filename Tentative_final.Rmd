...
get_taxa_unique(ps, "Family") #687
get_taxa_unique(ps, "Order") #416
get_taxa_unique(ps, "Kingdom") #2
ps <- subset_taxa(ps, Family !="Mitochondria")
ps <- subset_taxa(ps, Order !="Chloroplast")
ps <- subset_taxa(ps, Kingdom !="Eukaryota")
ps <- subset_taxa(ps, Kingdom !="NA")
get_taxa_unique(ps, "Family") #683
get_taxa_unique(ps, "Order") #413
get_taxa_unique(ps, "Kingdom") #2
ps
#22817 taxa and 57 samples
...
# READ IN OTU data that has been filtered for very low abundance sequences. 
otu <- read.table("ps5_silva_nochloronomito_otu_table_myorder.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("Panama_ps5_silva_nochloronomito_taxa_table.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("Panama_ps5_silva_metadata.txt",sep="\t",header=T,row.names=1)
...
sample_data(ps_ra_alam)$Treatment<-factor(sample_data(ps_ra_alam)$Treatment,levels=c("Resident","Control plots","Hypoxia plots"))
...
##put treatments in a different order
sample_data(ps_ra_ssid)$Treatment<-factor(sample_data(ps_ra_ssid)$Treatment,levels=c("Resident","Control plots","Hypoxia plots"))
...
cols<-c("Resident"="#E69F00","Control plots"="#56B4E9","Hypoxia plots"="#CC79A7")
sum_sig$Treatment<-factor(sum_sig$Treatment, levels=c("Resident","Control plots","Hypoxia plots"))
...
cols<-c("Resident"="#56B4E9","Control plots"="#E69F00","Hypoxia plots"="#CC79A7")
sum_sig$Treatment<-factor(sum_sig$Treatment, levels=c("Resident","Control plots","Hypoxia plots"))
fams <- ggplot(sum_sig, aes(x=Family, y=Proportion))+
  geom_point(size=4,aes(color=Treatment))+
  scale_colour_manual(values=cols)+
...
sig_sites <- glue::glue_collapse(droplevels(factor(res2$otu.names)), sep = ", ") #this is to get a list of the families that are different
print(sig_sites)
##Family_XII, Shewanellaceae, Rhodospirillales, JTB215, Marinobacteraceae, Pseudomonadaceae, Halomonadaceae, Marinifilaceae, Nitrincolaceae, Desulfovibrionaceae, Holosporaceae, Parvibaculaceae, Clostridiaceae_1, Francisellaceae, Midichloriaceae, Proteobacteria, Syntrophobacteraceae, Peptostreptococcaceae, Bacteroidales, Thermodesulfovibrionia, Cellvibrionaceae, Gammaproteobacteria, Nostocaceae, Sneathiellaceae, Thermoanaerobaculaceae, Desulfobacteraceae, Clostridiaceae_4, Spirochaetaceae, Marinilabiliaceae, Litoricolaceae, Thiotrichaceae
#Calculate relative abundance
datc_relabund <-  sweep(datc[,7:683], 1, rowSums(datc[,7:683]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
sig_dis <- select(datc_relnames, Sample, Species, Logger.MiniDOT, Family_XII, Shewanellaceae, Rhodospirillales, JTB215, Marinobacteraceae, Pseudomonadaceae, Halomonadaceae, Marinifilaceae, Nitrincolaceae, Desulfovibrionaceae, Holosporaceae, Parvibaculaceae, Clostridiaceae_1, Francisellaceae, Midichloriaceae, Proteobacteria, Syntrophobacteraceae, Peptostreptococcaceae, Bacteroidales, Thermodesulfovibrionia, Cellvibrionaceae, Gammaproteobacteria, Nostocaceae, Sneathiellaceae, Thermoanaerobaculaceae, Desulfobacteraceae, Clostridiaceae_4, Spirochaetaceae, Marinilabiliaceae, Litoricolaceae, Thiotrichaceae )
...
names(otud) <- make.names(names(otud)) #get the names from the table for 
otu_test <- otud #rename otud to otu_test, for syntax in ANCOM
metadat <- select(metadat, c("Sample.ID","Species","Logger.MiniDOT")) # use select to only use treatment columns of interest
map_test <- metadat #rename map_TEst
Vardat <- map_test #specify that this for Vardat - ANCOM syntax
#### ANCOM test - not adjusted, more than 2 levels = Kruskal Wallis
comparison_test_log=ANCOM.main(OTUdat=otu_test, #calling the OTU table
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
resALL3 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(resALL3,"SSID_ANCOM_family_ANOVA_Logger_covarSpecies.txt",sep="\t",col.names=NA)
resALL4 <- res[which(resALL3$detected_0.9==TRUE),] 
write.table(resALL4,"resALL4.txt",sep="\t",col.names=NA)
sig_sites <- glue::glue_collapse(droplevels(factor(resALL4$otu.names)), sep = ", ") #this is to get a list of the families that are different
print(sig_sites)
#Family_XII, Shewanellaceae, Rhodospirillales, JTB215, Marinobacteraceae, Pseudomonadaceae, Halomonadaceae, Marinifilaceae, Nitrincolaceae, Desulfovibrionaceae, Holosporaceae, Parvibaculaceae, Clostridiaceae_1, Francisellaceae, Midichloriaceae, Proteobacteria, Syntrophobacteraceae, Peptostreptococcaceae, Bacteroidales, Thermodesulfovibrionia, Cellvibrionaceae, Gammaproteobacteria, Nostocaceae, Sneathiellaceae, Thermoanaerobaculaceae, Desulfobacteraceae, Clostridiaceae_4, Spirochaetaceae, Marinilabiliaceae, Litoricolaceae, Thiotrichaceae, Idiomarinaceae, Marine_Group_II, Prolixibacteraceae, Archaea, B2M28, Arcobacteraceae, Desulfarculaceae, Schekmanbacteria, OCS116_clade, Cellvibrionales, Terasakiellaceae, Flammeovirgaceae, Desulfobulbaceae, Caenarcaniphilales, HOC36, Clade_III, Micrococcaceae, Rickettsiaceae, Anaerolineaceae, Moraxellaceae
#Calculate relative abundance
datc_relabund <-  sweep(datc[,7:683], 1, rowSums(datc[,7:683]), '/')
datc_relnames <- cbind(datc[,1:6],datc_relabund)
#only selet the significant families
...
```{r}
############ Pulling out important families
# aggregate ASVs by family
dat <- tax_glom(ps, taxrank = "Family")
#melt the data, so it's like a dataframe
datm <- psmelt(dat)
...
sig_sites <- glue::glue_collapse(droplevels(factor(resALL4$otu.names)), sep = ", ") #this is to get a list of the families that are different
print(sig_sites)
#Family_XII, Desulfovibrionaceae, Arcobacteraceae, Midichloriaceae
...
sub_pc_m$Treatment<-factor(sub_pc_m$Treatment, levels=c("Resident","Control plots","Hypoxia plots"))
cols<-c("Resident"="#56B4E9","Control plots"="#E69F00","Hypoxia plots"="#CC79A7")
